...
const mockApps: AppSubmission[] = Array.from({ length: env.MOCK_DATA_COUNT }, (_, i) => ({
  id: `app-${i + 1}`,
  name: `App ${i + 1}`,
  ...
  submittedAt: new Date(
    Date.now() - Math.random() * env.MOCK_SUBMISSION_DATE_RANGE_DAYS * 24 * 60 * 60 * 1000,
  ).toISOString(),
  updatedAt: new Date(
    Date.now() - Math.random() * env.MOCK_UPDATE_DATE_RANGE_DAYS * 24 * 60 * 60 * 1000,
  ).toISOString(),
  developer: {
